...
Original file is located at
    https://colab.research.google.com/drive/1vN1lBmNgcdyuPP3gAmcWEzKYWLnPKkVB
"""
...
kaze_train = get_kaze(train_files, name='kaze_train', save=True)
kaze_val = get_kaze(val_files, name='kaze_val', save=True)
kaze_test = get_kaze(test_files, name='kaze_test', save=True)

kaze_train = load_feature('cache/kaze_train.pkl')
kaze_val = load_feature('cache/kaze_val.pkl')
kaze_test = load_feature('cache/kaze_test.pkl')

kaze_train.shape, kaze_val.shape, kaze_test.shape

train_img= train_files[1].reshape(1,64,64,3)
kaze_1=get_kaze(train_img, name='kaze_1', save=True)
...
mlp = create_mlp(kaze_train.shape[1], regress=False)
...
model.fit(
	x=[kaze_train, train_files], y=train_labels,
	validation_data=([kaze_val, val_files], val_labels),
	batch_size=BATCH_SIZE, epochs=6,verbose=1)
# make predictions on the testing data
#print("[INFO] predicting house prices...")
#preds = model.predict([testAttrX, testImagesX])\
model.save('mymodel_malariamodkaze.h5')
from google.colab import files
files.download('mymodel_malariamodkaze.h5')

from tensorflow.keras.models import load_model
model=load_model("/content/mymodel_malariamodkaze.h5")

from keras.utils.vis_utils import plot_model
plot_model(model, to_file='model_kaze.png', show_shapes=True, show_layer_names=True)



print("Test_Accuracy: {:.2f}%".format(model.evaluate([kaze_test, test_files], np.array(test_labels))[1]*100))

preds = model.predict([kaze_test, test_files])
print(preds)
...
for i in range(len(test_files)):
    ...
    if(image_probs[0]>0.3 and image_probs[1]>0.3):
        ...
        kaze_test_filtered.append(kaze_test[i])
...
def create_adversarial_pattern(input_image, input_label):
  with tf.GradientTape() as tape:
    ...
    kaze_adv=get_kaze(image, name='kaze_adv', save=True)
    print("normal image kaze", kaze_adv)
    kaze_adv.reshape(1,2048)
    prediction = model([kaze_adv,input_image])
...
def display_images(image, description):
  data=np.array(image)
  data=data.reshape(1,64,64,3)
  kaze_adv=get_kaze(data, name='kaze_adv', save=True)
  print("adversarial kaze: ",kaze_adv)
  kaze_adv.reshape(1,2048)
  label=model.predict([kaze_adv,image])
...
def argmax(array):
  ...
  return index
X_test_adversary=[]
kaze_test_filtered=np.array(kaze_test_filtered)
X_test_filtered=np.array(X_test_filtered)
print(X_test_filtered.shape)
print(kaze_test_filtered.shape)

preds_filtered = model.predict([kaze_test_filtered,X_test_filtered])
...
for i in range(len(X_test_filtered)):
    ...
    #print(model.predict(np.array(data)))
    image=np.array(data)
    kaze_a=get_kaze(image, name='kaze_adv', save=True)
    image = tf.convert_to_tensor(image)
    #print("image",np.max(image))
    loss_object = tf.keras.losses.CategoricalCrossentropy()

    print("normal image kaze1:", kaze_a)
    #kaze = np.array([kaze(image, block_norm='L2') ])
    ...
    #kaze=kaze_test_filtered[i]
    #kaze=np.array(kaze)
    #kaze=kaze.reshape(1,2916)
