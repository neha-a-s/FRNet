...
Original file is located at
    https://colab.research.google.com/drive/1fnYyqCuQO39nMKAzDngwk-tRfweXYSzi
"""
...
def get_sift(images, name='sift', save=True):
...
for i in range(len(train_files)):
    train_files_8.append(cv2.normalize(train_files[i], None, 0, 255, cv2.NORM_MINMAX).astype('uint8'))

test_files_8 = []
for i in range(len(test_files)):
    test_files_8.append(cv2.normalize(test_files[i], None, 0, 255, cv2.NORM_MINMAX).astype('uint8'))

val_files_8 = []
for i in range(len(val_files)):
    val_files_8.append(cv2.normalize(val_files[i], None, 0, 255, cv2.NORM_MINMAX).astype('uint8'))

# Commented out IPython magic to ensure Python compatibility.
!pip install ipython-autotime
# %load_ext autotime

sift_train = get_sift(train_files_8, name='sift_train', save=True)
sift_val = get_sift(val_files_8, name='sift_val', save=True)
sift_test = get_sift(test_files_8, name='sift_test', save=True)

sift_train = load_feature('cache/sift_train.pkl')
sift_val = load_feature('cache/sift_val.pkl')
sift_test = load_feature('cache/sift_test.pkl')
...
mlp = create_mlp(sift_train.shape[1], regress=False)
...
model.fit(
	x=[sift_train, train_files], y=train_labels,
	validation_data=([sift_val, val_files], val_labels),
	batch_size=BATCH_SIZE, epochs=6,verbose=1)
# make predictions on the testing data
#print("[INFO] predicting house prices...")
#preds = model.predict([testAttrX, testImagesX])\
model.save('mymodel_malariamodsift.h5')
from google.colab import files
files.download('mymodel_malariamodsift.h5')

from tensorflow.keras.models import load_model
model=load_model("/content/mymodel_malariamodsift.h5")
...
print("Test_Accuracy: {:.2f}%".format(model.evaluate([sift_test, test_files], np.array(test_labels))[1]*100))

preds = model.predict([sift_test, test_files])
print(preds)
...
for i in range(len(test_files)):
    ...
    if(image_probs[0]>0.3 and image_probs[1]>0.3):
        ...
        sift_test_filtered.append(sift_test[i])
...
def create_adversarial_pattern(input_image, input_label):
  with tf.GradientTape() as tape:
    ...
    image1 = cv2.normalize(image, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
    sift_adv=get_sift(image1, name='surf_adv', save=True)
    print("normal image hog", sift_adv)
    sift_adv.reshape(1,1920)
    prediction = model([sift_adv,input_image])
...
def display_images(image, description):
  data=np.array(image)
  data=data.reshape(1,64,64,3)
  image1 = cv2.normalize(data, None, 0, 255, cv2.NORM_MINMAX).astype('uint8')
  sift_adv=get_sift(image1, name='hog_adv', save=True)
  print("adversarial hog: ",sift_adv)
  sift_adv.reshape(1,1920)
  label=model.predict([sift_adv,image])
...
def argmax(array):
  ...
  return index
X_test_adversary=[]
sift_test_filtered=np.array(sift_test_filtered)
X_test_filtered=np.array(X_test_filtered)
print(X_test_filtered.shape)
print(sift_test_filtered.shape)

preds_filtered = model.predict([sift_test_filtered,X_test_filtered])
...
for i in range(len(X_test_filtered)):
    ...
    surf_a=get_sift(image1, name='surf_adv', save=True)
    print("normal image surf1:", surf_a)
