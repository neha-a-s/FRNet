...
Original file is located at
    https://colab.research.google.com/drive/1FAXGBHWnFhMQP8ZVPCwYedELWkvF7cFc
"""
...
lbp_1=get_lbp(train_img, name='lbp_1', save=True)

# Commented out IPython magic to ensure Python compatibility.
!pip install ipython-autotime
# %load_ext autotime

lbp_train = get_lbp(train_files, name='lbp_train', save=True)
lbp_val = get_lbp(val_files, name='lbp_val', save=True)
lbp_test = get_lbp(test_files, name='lbp_test', save=True)
...
mlp = create_mlp(lbp_train.shape[1], regress=False)
...
model.fit(
	x=[lbp_train, train_files], y=train_labels,
	validation_data=([lbp_val, val_files], val_labels),
	batch_size=BATCH_SIZE, epochs=6,verbose=1)
# make predictions on the testing data
#print("[INFO] predicting house prices...")
#preds = model.predict([testAttrX, testImagesX])\
model.save('mymodel_malariamodlbp.h5')
from google.colab import files
files.download('mymodel_malariamodlbp.h5')

from tensorflow.keras.models import load_model
model=load_model("/content/mymodel_malariamodlbp.h5")

from keras.utils.vis_utils import plot_model
plot_model(model, to_file='model_lbp.png', show_shapes=True, show_layer_names=True)



print("Test_Accuracy: {:.2f}%".format(model.evaluate([lbp_test, test_files], np.array(test_labels))[1]*100))

preds = model.predict([lbp_test, test_files])
print(preds)
...
for i in range(len(test_files)):
    ...
    if(image_probs[0]>0.3 and image_probs[1]>0.3):
        ...
        lbp_test_filtered.append(lbp_test[i])
...
def create_adversarial_pattern(input_image, input_label):
  with tf.GradientTape() as tape:
    ...
    lbp_adv=get_lbp(image, name='lbp_adv', save=True)
    print("normal image hog", lbp_adv)
    lbp_adv.reshape(1,4096)
    prediction = model([lbp_adv,input_image])
...
def display_images(image, description):
  data=np.array(image)
  data=data.reshape(1,64,64,3)
  lbp_adv=get_lbp(data, name='lbp_adv', save=True)
  print("adversarial hog: ",lbp_adv)
  lbp_adv.reshape(1,4096)
  label=model.predict([lbp_adv,image])
...
def argmax(array):
  ...
  return index
X_test_adversary=[]
lbp_test_filtered=np.array(lbp_test_filtered)
X_test_filtered=np.array(X_test_filtered)
print(X_test_filtered.shape)
print(lbp_test_filtered.shape)

preds_filtered = model.predict([lbp_test_filtered,X_test_filtered])
...
for i in range(len(X_test_filtered)):
    ...
    #print("image",np.max(image))
    loss_object = tf.keras.losses.CategoricalCrossentropy()
    #lbp_a=get_lbp(image, name='lbp_adv', save=True)
    #print("normal image lbp1:", lbp_a)
